...
define(['socket.io', 'simulated_touch_factory', 'config', 'log', "prototype"], function (io, touchFactory, config, log) {
     ...
     * @param {Integer} value
     */
    function sendToDevice(event, type, value) {
        mSocket.emit(event, 
        {
            type: type,
            value: value
        });
        log.d(TAG, event + ":: " + type + ": " + value);
    }
    ...
    function parseTouchEvent(touches, defineStartPosition) {
        ...
        for (var i = 0, max = touches.length; i < max; i++) {
            ...
            if (touch.clientX > halfWindowWidth) {
                ...
                sendToDevice("rotation", "tilt_up_down", mTiltUpDown);
                sendToDevice("rotation", "tilt_left_right", mTiltLeftRight);
            } else {
                // left side
                ...
                sendToDevice("motors", "power", mPower);
                sendToDevice("rotation", "orientation", mOrientation);
            }
        }
